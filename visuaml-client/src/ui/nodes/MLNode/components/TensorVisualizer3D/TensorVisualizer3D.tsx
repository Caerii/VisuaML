    ...
    for (let x = 0; x < PREVIEW_GRID_SIZE; x++) {
      for (let y = 0; y < PREVIEW_GRID_SIZE; y++) {
        for (let z = 0; z < PREVIEW_GRID_SIZE; z++) {
          const position: [number, number, number] = [(x - 1) * spacing, (y - 1) * spacing, (z - 1) * spacing];
...
const ControlsOverlay: React.FC = () => (
  <div style={STYLES.controlsOverlay}>
    ...
    <div>• Scroll: Zoom</div>
    <div style={{ marginTop: '4px' }}><strong>📱 Touch:</strong></div>
...
const TensorSlices: React.FC<TensorSlicesProps> = ({ 
  ...
  const handlePointerOver = isFullscreen ? (e: ThreeEvent<PointerEvent>) => {
    e.stopPropagation();
    const point = e.point;
  ...
  return (
    <group>
      <instancedMesh
        ref={solidMeshRef}
        args={[getCachedGeometry(CONSTANTS.DIMENSIONS.VOXEL_SIZE), getCachedMaterial(0.7, 0.05), totalVoxelCount]}
...
const InfoCard: React.FC<InfoCardProps> = ({ shape, hoveredChannelIndex, sliceColors, isFullscreen }) => {
    ...
    <div style={STYLES.infoCard}>
      ...
      <div style={{ marginBottom: '8px' }}>
        ...
        <div style={{ marginLeft: '8px', fontSize: '12px', color: '#666' }}>
          ...
          • Colors represent different channels <br />
          • Stacked along depth (Z-axis)
        </div>
      </div>

      {isSimplified && (
        <div style={{ marginBottom: '8px', padding: '8px', background: 'rgba(245, 158, 11, 0.1)', borderRadius: '4px', border: '1px solid rgba(245, 158, 11, 0.2)' }}>
          <strong style={{ color: '#d97706' }}>⚠️ Simplified Rendering:</strong>
          <div style={{ fontSize: '12px', color: '#666', marginTop: '4px' }}>
            <div style={{ marginBottom: '6px' }}><strong>Original per channel:</strong> {height} × {width} = {originalElementsPerChannel.toLocaleString()} elements</div>
            <div style={{ marginBottom: '6px' }}><strong>Rendered per channel:</strong> {effectiveRenderHeight} × {effectiveRenderWidth} = {renderedElementsPerChannel.toLocaleString()} voxels</div>
            <div style={{ marginBottom: '6px' }}><strong>Total original:</strong> {totalOriginalElements.toLocaleString()} elements</div>
            <div style={{ marginBottom: '6px' }}><strong>Total rendered:</strong> {totalRenderedElements.toLocaleString()} voxels</div>
            <div><strong>Reduction:</strong> {Math.round((1 - renderedElementsPerChannel / originalElementsPerChannel) * 100)}% fewer elements per channel</div>
          </div>
        </div>
      )}

      {hoveredChannelIndex !== null && (
        <div style={{ marginTop: '12px', padding: '8px', background: 'rgba(37, 99, 235, 0.1)', borderRadius: '4px', border: '1px solid rgba(37, 99, 235, 0.2)' }}>
          ...
          <div style={{ fontSize: '12px', color: '#666', marginTop: '4px' }}>
            ...
            Elements: {isSimplified ? `${renderedElementsPerChannel.toLocaleString()} rendered (${originalElementsPerChannel.toLocaleString()} original)` : `${renderedElementsPerChannel.toLocaleString()}`}
          </div>
        </div>
      )}
    </div>
...
const TensorVisualizer3D: React.FC<UseTensorVisualizer3DReturn> = (props) => {
  ...
  if (!shape || shape.length !== 3) {
    return (
      <div style={{ width: '100%', height: height || '100px', display:'flex', alignItems:'center', justifyContent:'center', backgroundColor: '#e9ecef', fontSize: '0.8em', color: '#6c757d' }}>
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: height || '100%', marginBottom: marginBottom || 0 }}>
      <div 
        ref={fullscreenRef}
        style={{ width: '100%', height: isFullscreen ? '100vh' : '100%', backgroundColor: '#000', position: 'relative' }}
      ...
      >
        ...
        <Canvas 
          camera={{ position: cameraConfig.position, fov: CONSTANTS.CAMERA.FOV, near: 0.01, far: Math.max(cameraConfig.distance * 5, 100) }}
        ...
        >
          {isFullscreen ? (
            <>
              ...
              <pointLight position={[0, cameraConfig.distance / 1.5, cameraConfig.distance / 1.5]} intensity={0.6} distance={cameraConfig.distance * 3} decay={1.5} />
            </>
          ) : (
            <>
          ...
          <OrbitControls 
            ...
            mouseButtons={{ LEFT: THREE.MOUSE.ROTATE, MIDDLE: THREE.MOUSE.DOLLY, RIGHT: isFullscreen ? THREE.MOUSE.PAN : THREE.MOUSE.ROTATE }}
          />
        </Canvas>
      </div>
    </div>
